...
export default function AdminSensorHealth() {
  ...
  useEffect(() => {
    // 1. Database එකෙන් දැනට තියෙන සෙන්සර් ටික ගන්නවා
    const fetchSensors = async () => {
      try {
        const response = await axios.get(`${envUrl}/sensors`);
    ...
    // 2. Socket එකෙන් Live එන Health Updates අල්ලගන්නවා
    const socket = io(socketUrl, { transports: ['websocket'] });

    socket.on('sensor-health-update', (updatedSensor) => {
      setSensors((prevSensors) => {
        const exists = prevSensors.find(s => s.sensorId === updatedSensor.sensorId);
        if (exists) {
          return prevSensors.map(s => s.sensorId === updatedSensor.sensorId ? updatedSensor : s);
        } else {
          return [updatedSensor, ...prevSensors];
        }
      });
    });
      ...
      <div className="relative z-10">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-[#111826]/50 rounded-3xl border border-dashed border-slate-800">
            <p className="text-slate-500 font-bold uppercase tracking-widest">No Sensors Registered Yet</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            {sensors.map((sensor) => {
              ...
              const pingTime = new Date(sensor.lastPing).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                  ...
                  <div className="flex flex-col sm:flex-row gap-6 relative z-10">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6 mt-4 bg-slate-900/50 p-3 rounded-xl border border-slate-800/50">
                        ...
                        <div className="flex items-center gap-3 text-slate-400 text-sm truncate">
                          <FaWifi className={isOffline ? "text-red-500 shrink-0" : "text-green-500 shrink-0"} /> 
                          <span className="truncate">Last Ping: {pingTime}</span>
                        </div>
                        <div className="flex items-center gap-3 text-slate-400 text-sm truncate">
                          <FaBatteryHalf className={sensor.battery === "0%" ? "text-red-500 shrink-0" : "text-emerald-500 shrink-0"} /> 
                          <span className="truncate">Battery: <span className={sensor.battery === "0%" ? "text-red-400 font-bold" : ""}>{sensor.battery}</span></span>
