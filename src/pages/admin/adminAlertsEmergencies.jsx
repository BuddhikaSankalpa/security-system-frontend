...
export default function AdminAlertsEmergencies() {
  ...
  useEffect(() => {
    ...
    // Listen for global deletion to clear UI automatically if triggered by another admin
    socket.on('all-alerts-deleted', () => {
      setAlerts([]);
    });

    return () => {
      socket.off('new-emergency');
      socket.off('all-alerts-deleted');
    };
  }, []);
  ...
  const deleteAllAlerts = async () => {
    const isConfirmed = window.confirm("Are you sure you want to permanently delete ALL alerts from the database? This action cannot be undone.");
    if (!isConfirmed) return;

    try {
      setIsLoading(true);
      await axios.delete(`${envUrl}/alerts`);
      setAlerts([]); // Clear state immediately
      toast.success("All Records Successfully Cleared!");
    } catch (err) {
      console.log(err);
          ...
          <div className="flex flex-wrap gap-3 w-full md:w-auto">
            {hasPendingAlerts && (
              <button 
                ...
                className="px-6 py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white rounded-lg font-bold tracking-wide uppercase transition-all shadow-[0_0_15px_rgba(34,197,94,0.3)] flex items-center justify-center gap-2 flex-1 md:flex-none"
              >
            ...
            {alerts.length > 0 && (
              <button 
                ...
                className="px-6 py-2.5 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white rounded-lg font-bold tracking-wide uppercase transition-all shadow-[0_0_15px_rgba(225,29,72,0.3)] flex items-center justify-center gap-2 flex-1 md:flex-none"
              >
                <IoTrash size={20} /> Clear All Records
              </button>
            )}
          </div>
        </div>
